defmodule Lily.OperatorTest do
  ...
  describe "type" do
    test "raise if type is not :def or :defmacro" do
      assert_raise(OperatorError, ~r/expected type to be :def or :defmacro/, fn ->
  ...
  describe "atom" do
    test ~S/raise if in [:., :"=>", :^, :"not in", :when]/ do
      Enum.each([:., :"=>", :^, :"not in", :when], fn atom ->
        assert_raise(OperatorError, ~r/used by the Elixir parser/, fn ->
    ...
    test "raise if atom is not an operator" do
      assert_raise(OperatorError, ~r/expected an operator/, fn ->
